...
timeout 30 bash -c 'until php -r "\$pdo = new PDO(\"mysql:host=\" . \"'\"\$DB_HOST\"'\". \";port=\" . \"'\"\$DB_PORT\"'\", \"'\"\$DB_USERNAME\"'\", \"'\"\$DB_PASSWORD\"'\"); if (\$pdo) exit(0); else exit(1);" 2>/dev/null; do echo "Waiting for database..."; sleep 2; done'
